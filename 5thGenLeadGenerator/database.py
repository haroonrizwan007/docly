...
-- ---------------------------------------------------------------------
-- ROOFING — a niche-specific outreach tab, same proven model as
-- BulkReach (own tables, queued imports, daily limit, Day0/2/7 auto
-- follow-up, own CRM/tracking/reporting) but with a lower default daily
-- limit (20) and a manual "Send Mail" button that sends synchronously
-- with a live progress bar, instead of relying only on the background
-- scheduler for the initial Day 0 batch.
-- ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ROOFING — settings / templates (mirrors BulkReach's pattern exactly:
# separate Subject field per step + HTML body, since a pasted full HTML
# document can't double as a subject line the way Docly's plain text can)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ROOFING — import (contacts start QUEUED, not immediately due — same
# "release N per day" model as BulkReach). Duplicate emails are always
# skipped automatically (checked against normalized_email UNIQUE index).
# ---------------------------------------------------------------------------
...
def roofing_get_queued_candidates(limit: int) -> list[dict]:
    """Read-only: oldest-queued-first, without changing status."""
    if limit <= 0:
        return []
    with get_connection() as conn:
        rows = conn.execute(
...
def roofing_activate_sequences(seq_ids: list[int]):
    """Mark specific (already-selected) sequence rows ACTIVE with
    next_send_at=now, so they're immediately ready to send."""
    if not seq_ids:
        return
    now = _now()
    with get_connection() as conn:
...
def roofing_get_due_sequences(limit: int = 50) -> list[dict]:
    """ACTIVE sequences whose next_send_at has arrived (follow-ups, and
    anything just activated), oldest first. Never includes QUEUED."""
    now = _now()
    with get_connection() as conn:
        rows = conn.execute(
            """
